...
program
  ...
  .option(
    "--origins <urls...>",
    "additional origins to include (e.g. SSO/OAuth domains)",
  )
  .option("--names <names...>", "filter to specific cookie names")
  .option(
    "--browsers <browsers...>",
    "browser backends to try: chrome, edge, firefox, safari",
  )
...
const getCookiesOptions: GetCookiesOptions = {
  ...
  chromiumBrowser: opts.chromiumBrowser as
    | "chrome"
    | "brave"
    | "arc"
    | "chromium"
...
try {
  ...
  if (opts.output) {
    ...
    console.error(
      `Wrote ${cookies.length} cookie(s) to ${outPath}`,
    );
  } else {
